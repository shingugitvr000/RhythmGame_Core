...
public class SimpleSequenceEditor : EditorWindow
{
    ...
    {
        var winodw = GetWindow<SimpleSequenceEditor>();
        winodw.titleContent = new GUIContent("Simple Sequencer");
        winodw.Show();
    }
    ...
    private void InitializeTracks()                 //시퀸스 데이터가 들어왔을 때 데이터를 셋팅해주는 함수 
    {
        ...
        while(sequenceData.trackNotes.Count < sequenceData.numberOfTracks)          //트랙수 많큼 List<int>()를 넣어준다.
        {
            sequenceData.trackNotes.Add(new List<int>());
        }

        foreach(var track in sequenceData.trackNotes)                       //각 트랙 노트의 비트 수만큼 0 값으로 데이터를 넣어 준다. 
        {
        ...
        if(audioSource != null)                                     //가져온 시쿼스 데이터의 오디오 클립이 있을 경우 오디오 데이터를 할당한다.
        {
    ...
    private void Update()
    {
        ...
        if(isPlaying && audioSource!= null && audioSource.isPlaying)
        {
            ...
            currentBeatTime = Mathf.FloorToInt(elapseTime * sequenceData.bpm / 60f);    //BPM분의 60에 흐른 시간으 곱하면 현재 비트이다.

            if(currentBeatTime >= totalBeats)
            {
                StopPlayback();
            }
            Repaint();                                  //업데이트가 끝한후에 리페인트 (화면 갱신을 해주지 않는 업데이트이기 때문에)
        }
    }

    private void StartPlayback(int fromBeat)                        //오디오를 특정 비트부터 플레이 해주는 함수 
    {
        if (sequenceData == null || sequenceData.audioClip == null || audioSource == null) return;      //3가지중 하나라도 없으면 리턴
        ...
        float startTime = fromBeat * 60f / sequenceData.bpm;            //시작시간을 비트와 BPM으로 부터 계산하여 추정한다.
    ...
    private void DrawBeat(int trackIndex, int beatIndex)
    {
        ...
        bool isCurrentBeat = currentBeatTime == beatIndex;  //cuurentBeatTime 과 beatIndex가 동일하면 true 아니면 false
    ...
    private void OnGUI()
    {
        ...
        if(GUILayout.Button(isPlaying ? "일시 정지" : "재생"))
        {
            ...
            else StartPlayback(currentBeatTime);                //일시 정지면 다시 재생
        }
        if(GUILayout.Button("처음 부터 재생"))
        {
            StartPlayback(0);                                   //0번째 비틉부터 재생
        }
